...
async def cmd_start(message: types.Message):
    await message.reply("Welcome to Computer Networking Test Bot\n to start test click /test command from menu below!")
...
async def cmd_test(message: types.Message):
    ...
    d = load(data)
    tests = test_reader(d)
    await test(tests, message)


@dp.message_handler(commands="stop")
async def cmd_stop(message: types.Message):
    await message.answer("Session ends successfully", reply_markup=types.ReplyKeyboardRemove())


@dp.message_handler()
async def checker(message: types.Message):
    global correct, op
    while message.text not in ['A', 'B', 'C', 'D']:
        await message.answer("Try again\nYour answer: ")
    else:
        count = ord('A')
        for j in op:
            if j == correct:
                if chr(count) == message.text:
...
async def test(tests, message: types.Message):
    global op, correct
    i = choice(tests)
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    keyboard.add("A", "B", "C", "D")
    await message.answer(f"{i['question']}", reply_markup=keyboard)
    i['options'].append(i['correct'])
    count = ord('A')
    op = option_gen(i['options'])
    correct = i['correct']
    for j in op:
        await message.answer(f"{chr(count)}) {j}")
        count += 1
